...
export async function getStatus(slackMemberId: string): Promise<Status> {
    ...
    return { status_text: profile.status_text, status_emoji: profile.status_emoji };
}

export async function setStatus(slackMemberId: string, status: Status): Promise<void> {
